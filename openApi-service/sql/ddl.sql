-- 创建库
create database if not exists `open-api`;
...
(
    ...
    `accessKey`  varchar(512)                           not null comment '调用标识',
    `secretKey`  varchar(512)                           not null comment '调用秘钥',
...
-- 接口信息
create table if not exists `open-api`.`interface_info`
(
...
-- 用户调用接口信息表
create table if not exists `open-api`.`user_interface_info`
(
    `id`                 bigint                             not null auto_increment comment '主键' primary key,
    `userId`             bigint                             not null comment '调用者 id',
    `totalNum`           int      default 0                 not null comment '调用接口总数',
    `com.molloi.openApi` int      default 0                 not null comment '剩余调用次数',
...
-- 用户调用接口信息表
create table if not exists `open-api`.`user_interface_info_log`
(
    `id`              bigint                             not null auto_increment comment '主键' primary key,
    `userId`          bigint                             not null comment '调用者 id',
...
insert into `open-api`.`interface_info` (name, description, url, requestHeader, responseHeader, status, method, userId)
